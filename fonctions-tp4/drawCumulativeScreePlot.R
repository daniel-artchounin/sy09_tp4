drawCumulativeScreePlot <- function(princompResult, title, cex=1, round=2){
	...
	brplt <- barplot(screePlotData, main=title, names.arg=xLab, ylim=c(0, 100), ylab= "Pourcentage d'inertie expliquée par chaque chaque sous espace principal")
	text(x=brplt, y=screePlotData+3, labels=as.character(round(screePlotData, round)), xpd=TRUE, cex=0.2)
}